...
public class RepositoryExceptionAspect {
	...
	@AfterThrowing(value = "execution (* com.repositories.impl.*.*(..))", throwing = "e")
	...
	public Object daoExceptionIterceptor(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			String sb = (joinPoint.getTarget().getClass().getName() + " : " + Arrays.toString(joinPoint.getArgs()) + " in " + joinPoint.getSignature().getName()
					+ "()");
			iae.printStackTrace();
			logger.error(sb);
			throw iae;
