...
import com.repositories.InspectionPointExternalRepositoryCustom;
 ...
 */
@Repository
public class InspectionPointExternalRepositoryImpl extends JdbcDaoSupport implements InspectionPointExternalRepositoryCustom {


    @Autowired
    public InspectionPointExternalRepositoryImpl(DataSource dataSource) {
        super.setDataSource(dataSource);
    }

    @Override
    public List<InspectionPointExternal> getList(InspectionPointExternalSearchVO searchVO) {
        String sql = "SELECT  ipe.* from inspection_point_external ipe where 1=1 ";
        sql += createSearchSql(searchVO);//调用创建查询sql方法
        sql += " order by ipe.tag,ipe.displayOrder ";//拼接sql 排序
        ...
        if (count > 0) {
            ...
            return jdbcTemplate.query(sql, params, new BeanPropertyRowMapper<>(InspectionPointExternal.class));//执行sql 返回数据
        }
     ...
     * @return 查询sql
     */
    private String createSearchSql(InspectionPointExternalSearchVO searchVO) {
        String sql = "";
        if (StringUtil.isNotNullOrEmpty(searchVO.getExternalCode()))
            sql += " and ipe.externalCode = :externalCode";
        if (StringUtil.isNotNullOrEmpty(searchVO.getName()))
            sql += " and ipe.pointName like :nameParam";
        return sql;
    }
}
