...
import com.repositories.DictionaryTagRepositoryCustom;
...
/**
 * Created by 高宇飞 on 2017/8/31 11:32:29
 */
@Repository
public class DictionaryTagRepositoryImpl extends JdbcDaoSupport implements DictionaryTagRepositoryCustom {

    @Autowired
    public DictionaryTagRepositoryImpl(DataSource dataSource) {
        super.setDataSource(dataSource);
    }

    @Override
    public List<DictionaryTag> getList(DictionaryTagSearchVO searchVO) {
        String sql = "SELECT * from dictionary_tag WHERE 1=1 ";
        ...
        if (count > 0) {
            ...
            return jdbcTemplate.query(sql, params, new BeanPropertyRowMapper<>(DictionaryTag.class));//执行sql 返回数据
        }
     ...
     * @return 查询sql
     */
    private String createSearchSql(DictionaryTagSearchVO searchVO) {
        String sql = "";
        if (StringUtil.isNotNullOrEmpty(searchVO.getTag())) {
            sql += " and tag like :tagParam";
        }
        return sql;
    }
}
