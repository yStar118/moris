...
import com.repositories.ContingencySceneRepositoryCustom;
 ...
 */
@Repository
public class ContingencySceneRepositoryImpl extends JdbcDaoSupport implements ContingencySceneRepositoryCustom {

    @Autowired
    public ContingencySceneRepositoryImpl(DataSource dataSource) {
        super.setDataSource(dataSource);
    }

    @Override
    public List<ContingencyScene> getList(ContingencySceneSearchVO searchVO) {
            String sql = "SELECT cs.*,(SELECT username FROM sys_user WHERE id=cs.createUser) AS  'createUserName'," +
                    "(SELECT username FROM sys_user WHERE id=cs.updateUser) AS  'updateUserName'" +
                    " FROM contingency_scene cs where 1=1  ";
            sql += createSearchSql(searchVO);//调用创建查询sql方法
            sql += " order by cs.createDate desc ";//拼接sql 排序
            ...
            if (count > 0) {
                ...
                return jdbcTemplate.query(sql, params, new BeanPropertyRowMapper<>(ContingencyScene.class));//执行sql 返回数据
            }
         ...
         * @return 查询sql
         */
    private String createSearchSql(ContingencySceneSearchVO searchVO) {
        String sql = "";
        if (StringUtil.isNotNullOrEmpty(searchVO.getName()))//判断是否为null或者""
            sql += " and  cs.name like :nameParam";//注入参数  这里用的模糊查询  防注入 所以在bean里面拼接
        return sql;
    }
}
