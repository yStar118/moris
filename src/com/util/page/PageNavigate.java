...
public class PageNavigate {
	...
	private void buildNumModel() {
		...
		if (recordCount != 0) {
			...
			if(countPage >1){
				//新增指定跳转页面
				strHtml.append("<li><input type=\"text\"data-recordCount=\"").append(recordCount).append("\" data-maxpage=\"").append(countPage).append("\" data-url=\"").append(url)
						.append("\" style=\"width: 60px;height:33px;margin-left:5px;\" id=\"inputcount\" value=\"\"><button type=\"button\" onclick=\"pageHref()\"" +
								" class=\"btn  btn-sm  btn-primary\" style=\"margin-top:-3px;\">跳转</button></li>");
			}
