...
<html lang="en">
<head>
    <title>${webTitle }-检查结果检查点统计</title>
...
<script>
    $(function () {
        ...
        window.table = $table_id.DataTable({
            ...
            ajax: function (data, callback) {//ajax配置为function,手动调用异步查询
                $.ajax({
                    type: "GET",
                    url: '${staticServer}/portal/stat/checkResult/getListForPoint.do',//请求数据的参数
    ...
    /*导出*/
    function prints() {
        var url = "${staticServer}/portal/stat/checkResult/print.do?type=4&index=0&length=99999";
        ...
        if ($("#objectNameSearch").val() != '')
            url += "&objectName=" + $("#objectNameSearch").val();
        if ($("#pointNameSearch").val() != '')
            url += "&pointName=" + $("#pointNameSearch").val();
        if ($("#startDate").val() != '')
