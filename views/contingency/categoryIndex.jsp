                                        ...
                                        <div class="row">
                                            <div class="col-xs-3">
                                                <div class="form-group">
                                                    <label class="control-label col-xs-6 col-lg-5">分类名称：</label>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <table id="category" class="table table-striped table-bordered table-hover">
                                    <thead>
                                    <tr>
                                        <th style="background-color: #E5E5E5">分类编号</th>
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <form id="categoryForm" name="categoryForm" class="form-horizontal"
                              action="${staticServer}/portal/contingency/category/save.do" method="post">
...
<script>
    $(function () {
        ...
        window.table = $table_id.DataTable({
            ...
            ajax: function (data, callback) {//ajax配置为function,手动调用异步查询
                $.ajax({
                    type: "GET",
                    url: '${staticServer}/portal/contingency/category/getList.do',//请求数据的参数
                    data: categoryParam.getQueryCondition(data),
                    cache: false,  //禁用缓存
                    dataType: "json",
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn  btn-sm btn-info" href="#modal-edit" data-toggle="modal"  data-backdrop="static" ' +
                            'onclick="detailCategory(\'' + data + '\')">' +
                            '<i class="ace-icon fa fa-pencil-square-o "></i>详情</a>' +
                            '&nbsp;&nbsp;' +
                            '<a class="btn  btn-sm btn-success" href="#modal-edit" data-toggle="modal"  data-backdrop="static" ' +
                            'onclick="updateCategory(\'' + data + '\')">' +
                            '<i class="ace-icon fa fa-pencil-square-o "></i>修改</a>' +
                            '&nbsp;&nbsp;' +
                            '<a class="btn btn-sm btn-danger" onclick="delCategory(\'' + data + '\')">' +
    ...
    function updateCategory(id) {
        ...
        $.ajax({
            type: "GET",
            url: "${staticServer}/portal/contingency/category/findOne.do",
            data: {
                "id": id
            },
    ...
    function delCategory(id) {
        bootbox.confirm("<a style='font-size: 17px;color: red'>确定要删除该应急预案分类？</a>", function (result) {
            if (result) {
                $.ajax({
                    type: "GET",
                    url: "${staticServer}/portal/contingency/category/delete.do",
                    data: {
                        "id": id
                    },
                    contentType: 'application/json',
