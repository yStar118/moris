                            ...
                            <div class="widget-body">
                                <div class="widget-main">
                                    <table class="searchField" style="margin: 4px; padding: 4px;">
                                        <tr>
                                            <td>账号：</td>
                                            <td><input type="text" id="txtUsername" class="form-control" placeholder=""
                                                       value="${sysUserSearchVO.username }"></td>
                                            <td>姓名：</td>
                                            <td><input type="text" id="txtRealname" class="form-control" placeholder=""
                                            ...
                                            <td><form:select path="sysUserSearchVO.status" class="form-control"
                                                ...
                                                <form:option value="101">已删除</form:option>
                                            </form:select></td>
                                            <td>角色：</td>
...
<script type="text/javascript">
    ...
    var delUser = function (id) {
        bootbox.confirm("你确定要删除该用户吗？", function (result) {
            if (result) {
                window.location = "delete.do?id=" + id + "&backUrl=${backUrl}";
            }
        })
    };
    // 锁定
    var lock = function (id) {
        bootbox.confirm("你确定要锁定该用户吗？", function (result) {
            if (result) {
                window.location = "saveLock.do?id=" + id + "&backUrl=${backUrl}";
            }
        })
    };
    // 解锁
    var unlock = function (id) {
        bootbox.confirm("你确定要解锁该用户吗？", function (result) {
            if (result) {
                window.location = "saveUnlock.do?id=" + id + "&backUrl=${backUrl}";
            }
        })
    };
    // 解锁
    ...
    var resetPass = function (id) {
        bootbox.confirm("你确定要给该用户重置密码吗？", function (result) {
            if (result) {
                window.location = "saveResetPass.do?id=" + id + "&backUrl=${backUrl}";
            }
        })
    };
    ...
    var addUser = function (id) {
        window.location = 'toAdd.do?backUrl=${backUrl }';
    }
</script>
</body>
</html>
