    ...
    <div class="main-content">
                                    ...
                                    <table class="searchField" style="margin: 4px; padding: 4px;">
                                        <tr>
                                            <td>
                                                ...
                                                <c:if test="${bln:isP('SysFunctionAdd')}">
                                                ...
                                                <button type="button" class="btn  btn-sm"
                                                        onclick="location.href='${dynamicServer}/portal/sys/module/index.do'">
                        ...
                        <table id="simple-table" class="table table-striped table-bordered table-hover">
                            ...
                            <c:forEach items="${functionList}" var="sysFunction" varStatus="st">
                                <tr>
                                    ...
                                    <td><c:if test="${sysFunction.type eq 0}">读</c:if> <c:if
                                            test="${sysFunction.type eq 1}">写</c:if></td>
                                    <td>${sysFunction.display_order}</td>
                                    <td><c:if test="${bln:isP('SysFunctionAdd')}">
                                        <a href="toUpdate.do?id=${sysFunction.id}&backUrl=${backUrl}"> 修改</i>
                                        </a>
                                    </c:if><%-- <c:if test="${bln:isP('SysFunctionDelete')}">
                                        <a href="javascript:delFunction(${sysFunction.id }${sysFunction.module_id });"> 删除 </a>
                                    </c:if>--%></td>
                                </tr>
                            </c:forEach>
                            </tbody>
        ...
        <script type="text/javascript">
            ...
            var searchFunction = function () {
                var url = "index.do?module_id=${sysModule.id}";
            ...
            var addFunction = function (id) {
                window.location = 'toAdd.do?module_id=${sysModule.id}&backUrl=${backUrl }';
            }
        </script>
</body>
</html>
